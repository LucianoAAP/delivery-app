...
describe('Testa ReadAllSales', () => {
  ...
  before(async () => {
    ...
    await chai.request(app).post('/sales').send(sale).set('authorization', token);
  ...
  it('Retorna a lista de vendas correta', () => {
    ...
    expect(response.body[0].userId).to.be.equal(sales[0].userId);
  });
});
